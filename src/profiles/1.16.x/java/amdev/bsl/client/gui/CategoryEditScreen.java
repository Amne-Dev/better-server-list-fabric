...
public final class CategoryEditScreen extends Screen {
	...
	protected void init() {
		...
		this.categoryField = this.addButton(new EditBox(this.font, centerX - fieldWidth / 2, centerY - 10, fieldWidth, 20, new TranslatableComponent("bsl.gui.category")));
		this.categoryField.setMaxLength(32);
		this.categoryField.setValue(this.initialValue);
		this.categoryField.setFocus(true);

		this.addButton(new Button(centerX - 155, centerY + 22, 100, 20, new TranslatableComponent("bsl.button.save"), button -> this.bsl$saveAndClose()));
		this.addButton(new Button(centerX - 50, centerY + 22, 100, 20, new TranslatableComponent("bsl.button.clear"), button -> this.bsl$clearAndClose()));
		this.addButton(new Button(centerX + 55, centerY + 22, 100, 20, new TranslatableComponent("gui.cancel"), button -> this.minecraft.setScreen(this.parent)));
	}
